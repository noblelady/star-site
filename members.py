...
from semesters import FIRST_SEMESTER, get_current_semester, get_all_semesters, prev_semester, next_semester, \
    semester_pretty
...
class MemberListPage(webapp2.RequestHandler):
    def get(self, args):
        ...
        template_vals['members'] = Member.query(Member.show == True, Member.semesters_paid == selected_semester).order(
            Member.name).fetch(limit=None)
        ...
        template_vals['prev_semester'] = prev_semester(
            selected_semester) if selected_semester != FIRST_SEMESTER else None
        template_vals['next_semester'] = next_semester(
            selected_semester) if selected_semester != get_current_semester() else None
...
class HiddenListPage(webapp2.RequestHandler):
    def get(self):
        ...
        template_vals['members'] = Member.query(ndb.OR(Member.show == False, Member.never_paid == True)).order(
            Member.name).fetch(limit=None)
...
class MemberEditPage(webapp2.RequestHandler):
    ...
    def post(self, args):
        ...
        member.semesters_paid = [float(semester) for semester in
                                 self.request.get('semesters_paid', allow_multiple=True)]
